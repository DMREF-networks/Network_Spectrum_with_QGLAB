...
Also included is QGLAB.cmd, which is an exmaple slurm script for running this code on a computer cluster.
...
equi - (string) 'True' if the network is equilaterial, 'False' if not.\
p1 - If generating a URL this is a float and the value of a used in the construction of the point pattern. If loading a configuration this is a string and the path to the configuration file. If generating a lattice this is a string and denotes the kind of lattice, including:\
  ...
  2 - 'Hex' for the haxagonal lattice.\
...
2 - A .txt file containing the pairs indices of vertices that make up the edges of the network and their Euclidean lengths generated by NetGen.py placed in the configs/edgelists directory.\
